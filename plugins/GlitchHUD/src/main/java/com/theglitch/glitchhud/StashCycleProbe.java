...
public final class StashCycleProbe {
    ...
    /** @deprecated use {@link #getMillisUntilNextCycle(String)} — probes the first configured red world's scheduler only. */
    @Deprecated
    public static long getMillisUntilNextCycle() {
        return getMillisUntilNextCycle(null);
    }

    /** Millis until next extraction cycle for world (or the first configured red world if null/unmatched), or -1 if unavailable. */
    public static long getMillisUntilNextCycle(String world) {
        try {
            ...
            // Prefer the per-world accessor so each red world's own cycle timing is used.
            if (world != null && !world.isBlank()) {
                for (String m : SCHEDULER_NAMES) {
                    try {
                        java.lang.reflect.Method method = inst.getClass().getMethod(m, String.class);
                        scheduler = method.invoke(inst, world);
    ...
    /** @deprecated use {@link #formatNextCycle(String)} — probes the first configured red world's scheduler only. */
    @Deprecated
    public static String formatNextCycle() {
        return formatNextCycle(null);
    }

    public static String formatNextCycle(String world) {
        long ms = getMillisUntilNextCycle(world);
        if (ms < 0) return null;
        long s = ms / 1000;
        long m = s / 60;
        long sec = s % 60;
