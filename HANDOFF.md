...
- GlitchQuests: `/rewards` (daily streak/weekly/monthly) + `/quests` (5 daily of 12, 3 weekly of 7) with GUI-pack glyph backgrounds. Config `plugins/GlitchQuests/src/main/resources/config.yml` (live copy is seeded once — edit live + `/questadmin reload`). Built + deployed 2026-09-23.
...
- Custom UI theming 2026-08-23: Nexo font glyphs `E040-E049` (`server/plugins/Nexo/glyphs/oraxen_glyphs/theglitch.yml`, textures via `scripts/gen-ui-textures.py`), Wynncraft-style gear detail pages (`GlitchUI.java` + `GearManager.buildItem`), glyph titles in Shops/Hideout Java + Stash/Classes configs. Bedrock intentionally sees plain text. Live config patch needed once: stash display-name + classes gui.title (seeded-once files). Oraxen `itemname:` migration 2026-09-02 (20 items, displayname:→itemname: in 6e2fba7, config-version 3). **Chest-window art 2026-09-22 (rounds 8-9):** all 6 chest sizes are now themed (`generic_{9,18,27,36,45,54}.png`, modern + legacy paths), not just 54-slot ones. Round 8 copied the operator-supplied "Medieval" GUI kit's PNGs verbatim, which turned out to be drawn centered in their canvas instead of flush at vanilla's real (0,0) origin — scrambled every item icon in-game. Round 9 reverted to procedural generation (`chest_window()` in `scripts/gen-ui-textures.py`, pinned to vanilla's exact coordinates) recolored to the kit's wood/parchment palette. **Do not re-copy `assets/ui-kits/medieval/generic_*.png` directly into the pack — same bug.** The staged menu/jobs/profile/rewards template art there likely has the same centering issue and would need the same treatment before use. See docs/STATUS.md rounds 8-9 for the full writeup and the known glyph/inventory-screen color mismatch.
...
- GlitchShops is deployed and live-tested: `/shop` buy/sell works. Grand Bazaar NPC placement and balance tuning remain. **2026-09-23 (round 11):** added a 6th `mystic` category (25 fixed prestige weapons, priced above Legendary godroll) and generic pagination for any shop category over 20 items (`ShopGUI.java` — prev/next at slots 45/53). This is a real Java change, not asset-only — needs a jar rebuild + restart, unlike most of this session's other GUI/icon rounds. See docs/STATUS.md round 11 + docs/ITEM_SYSTEM.md §11.
