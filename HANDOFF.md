...
- GlitchHealthBar: removed entirely 2026-09-22 (see top of file). Mob health bars now come from the `littleroom healthbar` MythicMobs pack (switched from MythicMobs' own native `HealthBar` field the same day — see docs/STATUS.md/docs/MODELS.md). Its listener is world-scoped and spawned live (not config), one per active world — needs re-spawning after a restore that doesn't carry over entities. Two live bugs fixed the same day (round 3): bar not following a moving mob (self-healing re-mount every 4 ticks) and mid-body rendering on tall custom-modeled mobs (per-mob-type offset override, GlitchReaver's value is an untuned estimate) — see docs/MODELS.md.
- Party HUD: MythicHUD's party template can only resolve "who is member N" through a supported party plugin's own relational placeholders — no amount of custom `%glitchraid_*%` placeholder tuning could work around that. Fixed 2026-09-22 (round 3) by installing the **Parties** plugin (`server/plugins/Parties.jar`, gitignored, from AlessioDP's official file server — the raw Maven module jar is compile-only and won't load standalone) and adding `PartiesBridge.java` (GlitchRaid) to one-way-mirror GlitchRaid party membership into it. `partyhud-p.yml` now reads `%parties_list_rank_leader_1%` / `%parties_list_rank_member_1-3%`. `parties.user.*` was granted to the default LuckPerms group via RCON but the grant is unverified (LuckPerms' command feedback doesn't route through RCON) — worth checking `/lp group default info` in-game. Not yet confirmed working with a live player.
