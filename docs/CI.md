...
- The reactor covers all **10** modules: **9** deployable plugins + the GlitchCommon library. (GlitchDungeons, GlitchHUD, GlitchHealthBar, and GlitchLoot were removed 2026-09-22 — see docs/STATUS.md — in favor of MythicDungeons, MythicHUD, MythicMobs' native `HealthBar`, and plain MythicMobs DropTables.) `scripts/build-all.sh` (no args) builds/deploys the 9 deployable plugins; GlitchCommon builds only when something depends on it or via full-reactor fallback. It no longer syncs any TAB/Nexo HUD-takeover extras — that logic was GlitchHUD-specific and was removed with it. It does NOT sync balance/armor live configs — GlitchItems config v3 + Nexo itemname + MythicMobs COINS + scatter require manual diff via `scripts/deploy-balance-2026-09-02.sh` / `scripts/deploy-armor-2026-09-02.sh` + `mm reload` + `nexo reload`.
