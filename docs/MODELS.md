...
| Mob health bars: `littleroom healthbar` pack | 2026-09-22: replaced MythicMobs' native per-mob `HealthBar` field (which only drew a bar once a mob was damaged, and several users found it too easy to miss) with the `LITTLEROOM_healthbar_v1_0` pack — a global-listener-driven system (`server/plugins/MythicMobs/Packs/littleroom healthbar/`) using text-display entities, configurable bar/frame/icon style, status-effect icons, and floating damage numbers. Also puts a bar above **players** below 99% HP for anyone nearby — a real PvP-visibility change. **The pack's listener only scans its own world** (`@EIW` targeter) — on this multi-world server it needs one `HEALTHBAR_GLOBAL_LISTENER` spawned per active world (hub, glitch_pve, glitch_red, glitch_red_eleria, glitch_red_horizons); this is a live-spawned entity, not captured by any config file, so it must be re-spawned (`/mm mobs spawn HEALTHBAR_GLOBAL_LISTENER 1 <world>,0,100,0,0,0`) after any full world/server restore that doesn't carry over existing entities. Fixed two bugs in the downloaded pack itself (a metaskill reference the pack never defined, and a `/gamerule` command that errored on this server) — see commit 265ebf1. **2026-09-22 (round 3):** attempted a fix for the bar not tracking a moving mob by re-affirming the pack's `mountme` vehicle-mount every 4 ticks — **live playtest (round 4) showed this didn't work at all**, the bar still didn't move. **2026-09-22 (round 4):** replaced vehicle-mounting entirely — `HEALTHBAR_ENTITY_determine_status` now runs `teleportto{location=@ParentLocation{y=<vertical_offset>}}` every 2 ticks, reading the mob's live location directly instead of relying on Bukkit passenger mechanics. `vertical_offset` is redefined from "small offset above eye location" to "full height above the mob's feet" (default raised 0.25→2.0) to match the new anchor point; GlitchReaver's per-mob override (`HEALTHBAR_ENTITY_tall_mob_offsets`) raised to 3.0 accordingly — still an estimate, not measured against the live model. Not yet confirmed with a live player. |
