...
**Status as of 2026-08-10:** Phases 0-2 are implemented. Phase 3.1 is configured, but the live Bedrock join test is pending. Phase 4 mechanics are scripted; the default setup creates generated worlds, while imported map saves are external/live-only. Phase 5 has a broad source-level foundation: GlitchStash (incl. Fast/Silent extraction variants), GlitchClasses (full abilities + ultimates + starter kit), GlitchItems (identify, Residual Glitch consumers, loot containers), GlitchShops (live-tested), GlitchHealthBar (live-tested), GlitchDeathRules, and GlitchHideout. GlitchItems/GlitchShops/GlitchHealthBar are deployed; the rest of the recent source still needs live build/test (see [docs/TESTING.md](docs/TESTING.md)). GlitchDungeons remains deferred. The repository Standard timer is 30 seconds; generated live arena values still require verification. Oraxen has 18 item definitions and assets. See [docs/STATUS.md](docs/STATUS.md) for the authoritative status. EssentialsX is incompatible with the current Minecraft target. The immediate priorities are live verification, the Identifier NPC flow, physical world content, and end-to-end testing.
...
- [ ] **5.8 Extraction mechanic** — VelKoth (KOTH plugin in CAPTURE mode for extraction zones). _Repository config uses a 30s Standard timer. Fast/Silent variants implemented in source (GlitchStash, 2026-08-06): key-requiring zones, arming, payout bonus — live Fast/Silent arenas (15s/10s) must be created on the box. Generated `arenas.yml`, live arena values, and a full extraction test still need verification. Wand fix: click the block at your feet, not the ground below._
...
- [x] **5.9.2 GlitchClasses** — Class selection + abilities system. _Source core exists with four classes, selection GUI, ability items, persistence, and event listeners. Abilities + ultimates + reset costs completed in repo (2026-08-10); live verification remains pending._
...
- [ ] **5.10.7 World population** — spawners, chests, regen emitting rifts from 5.10.4. _DONE in repo (2026-08-10): glitch_red SpawnAreas seeded (T1 fodder everywhere, T2 mid cross-ring, T3 elite guards at Core 0,0 + extraction beacons X1/X2/X3; bootstrap.sh now seeds Spawners/SpawnAreas) + GlitchItems container system (Debris/Cache/Vault/Rift Vault marked per-block, key consumption, regen cooldown, loot-luck consumer). Remaining: mark containers in-world (operator) + live test._
...
- [ ] **5.11.6 Anti-grief / fair play** — GAME_DESIGN §9: friendly fire off everywhere, 2-min AFK kick in dungeons, shards account-bound (Coins currently drop on death as items — verify against "not tradeable"). _30s invulnerability on Red Zone entry points is DONE in GlitchDeathRules (2026-08-06): applies on world entry/join/respawn, optional entry-point zones, cancel-on-action, glow indicator._
