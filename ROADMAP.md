...
**Status as of 2026-09-02:** Phases 0-2 implemented; 3.1 configured (Bedrock join pending); Phase 4 mechanics scripted (generated vs imported saves external). **Phase 5.9 complete in source** — all seven extraction plugins + **Phase 5.7/5.8 complete**: **GlitchHUD** (per-world sidebar `NumberFormat.blank`, `BELOW_NAME` stacks, `NOTCHED_10` residual bar, TAB takeover) and **dynamic extraction** (`DynamicExtractionManager` + `SpotPicker` + `WaypointBridge`/`ExtractionMarkers` + `AutoExtractScheduler` 30m cycle, 3 validated random `extraction_dyn*` arenas, universal keys, force-loaded chunks) are built via the **14-module** Maven reactor and deployed live. All **12** deployable plugins + shared `GlitchCommon` load clean (`2026-09-02` `BUILD SUCCESS` — `f312262` GlitchHUD, `c1634b3`/`9d8f05a` dynamic extraction + fixes, `c9a229e` ping/TPS + `ByteTag` PDC crash fix, `4d8c554`/`6e2fba7`/`f1da4d0`/`d847c69` balance+armor — live cycle `3/3 at (670,299),(69,569),(299,942)`). In-game testing per [docs/TESTING.md](docs/TESTING.md) remains for newer plugins; `GlitchItems`/`GlitchShops`/`GlitchHealthBar` have full live tests, GlitchHUD visuals + dynamic capture/particles verified in-game. `GlitchDungeons` deferred. Oraxen 20 items (`itemname:` migrated from `displayname:` in 6e2fba7, config-version 3) + UI glyphs (`E040-E049` + `negative_space.json`). **Balance+armor deployed 2026-09-02 (4d8c554/6e2fba7/f1da4d0/d847c69, docs/ITEM_BALANCE.md):** 20 items, COINS retuned T1 1-2 / T2 Stalker 2-6 / Phantom 3-8 / Brute 5-10 / T3 10-16 / boss 40-80, roll-based gear sell (base 3/17/75/350/1750 + stars×bonus 2/8/25/90/350), weapons 4 attrs (lifesteal/fire-aspect/execute/frost-touch, Legendary 2 distinct) / armor 3 attrs (damage-reduction/thorns/glitch-ward, exactly 1), staff +2/3/5/7/9 ATTACK_DAMAGE / greatblade KB +0.1/0.15/0.2/0.25/0.3, consumables live (healing_potion/corrupted_heal + 25% boss drop, Rift Attunement Pack free any rarity, Void Infusion Epic+ boost+reroll), Vault +5% legendary rift, scatter rift_vault 10→6 (141 total), GlitchItems config-version 1→3, armor +0..+5 at Workbench ANVIL slot 40 or /armor upgrade (+1 armor/level, shards 10/25/60/150/400 × [1,2,3,4,6] + materials, per-slot identity) RCON verified. Root `pom.xml` resolves `me.clip:placeholderapi:2.12.3` via `https://repo.extendedclip.com/content/repositories/placeholderapi/`. Coins account-bound (`player-drop:false` …). EssentialsX incompatible. See [docs/STATUS.md](docs/STATUS.md) + [docs/ITEM_BALANCE.md](docs/ITEM_BALANCE.md).

**Status as of 2026-09-14:** + custom ModelEngine rigs live (R4.1.0 free, 2/12 slots) — Glitch Warden furnace golem (user `walk` clip, dual-path usm trigger, feet planted) and winged Glitch Wisp (static glide), blueprints tracked, merged pack, `3 models loaded` clean (`c32baa0`/`4d73404`, docs/MODELS.md) — visual sign-off pending; + pre-alpha hardening (raid-buffer red-entry block, repo-wide dialog removal, floating shop/dungeon panels) + rank ladder (`64ed02a`).
...
- [ ] **4.7 Red Zone points of interest** — physical structures at the Core (0,0 — Tier 4/5 loot), the 6 entry points, and the extraction sites. Entry coordinates documented (docs/ZONES.md); extraction sites are operator-placed in-game (VelKoth arenas). **2026-09-21:** two more Red Zone worlds added (`glitch_red_eleria`/`glitch_red_horizons`, hub picker GUI `/redzone`) — same open item applies to all three; see docs/ZONES.md's terrain-safety note (Horizons spawn moved off a lava tile).
...
- [x] **5.3 MythicMobs** — Custom mobs with Glitch Shards loot. _Done: plugin added to bootstrap.sh, 4 mob definitions (Glitch Stalker, Brute, Phantom, Core boss) with drop tables using COINS type. Configs seeded once. **2026-09-14:** ModelEngine R4.1.0 rigs attached (warden + wisp, docs/MODELS.md). **2026-09-21:** rigs removed at operator request — both mobs back to vanilla appearance. **2026-09-21 (later):** full-adopt custom rigs applied to all 8 non-boss mobs + 2 new mobs (GlitchReaver mini-boss, GlitchHarrower), sourced from 3 downloaded packs — 10/12 free-tier slots used, docs/MODELS.md/GAME_DESIGN.md §2 updated._
...
§10 list. Custom items ran on Oraxen (free, built from source) through
2026-09-19; migrated to **Nexo** 2026-09-20 (paid, ~€20, purchased + uploaded
manually like MythicMobs/ModelEngine — see docs/STATUS.md) for its tighter
MythicCrucible integration. No item levels: power comes from rarity tiers +
random stat rolls + Resonance matching, not number inflation._
...
- [x] **5.10.2 Material + key items** — 5 materials (Rune Fragment, Aether Shard, Rift Crystal, Void Essence, Legendary Relic) + 4 keys (Cache/Vault/Rift/Fast Extract). _Oraxen configs (`itemname:` — migrated from `displayname:` in 6e2fba7, 20 files, config-version 3 — GlitchItems config-version bumped 1→3), generated 16×16 textures, ESC-menu language override, and deployment script exist. Extended: 10 more item definitions (5 Unstable Rifts, Fast Extract Key, Healing Potion, Corrupted Heal, Rift Attunement Pack [was Rift Reveal Pack], Void Infusion) +2 new Aether Tonic [2026-09-02] + Ward Salve [2026-09-02] with sell-price lore — 20 custom items total. `displayname:`→`itemname:` (20 files) migrated 2026-09-02, `itemname:` format. Live deployment must be verified on the server (deployed 2026-09-02 via `scripts/deploy-balance-2026-09-02.sh`, `oraxen reload all` + restart, RCON verified)._
- [x] **5.10.3 Rarity tiers + stat-roll engine** — Common/Uncommon/Rare/Epic/Legendary; identify-outcome stat rolls (GlitchItems custom plugin). _Source V1 exists: gear generation, 3 archetypes, 4 armor pieces, stat ranges, stars, attributes, `/identify`, Resonance math, and Residual Glitch. Deployed and live-tested (2026-08-03): `/identify` works (retuned 2026-09-02, docs/ITEM_BALANCE.md). Mob integration complete (tags + drops, 2026-08-03; COINS retuned 2026-09-02 T1 1-2 T2 2-6/3-8/5-10 T3 10-16 boss 40-80); container integration complete (2026-08-10). **Retuned 2026-09-02 (docs/ITEM_BALANCE.md):** roll-based gear sell (base 3/17/75/350/1750 + stars×bonus 2/8/25/90/350), weapons 4 attrs (lifesteal/fire-aspect/execute/frost-touch, Legendary 2 distinct) / armor 3 attrs (damage-reduction/thorns/glitch-ward, exactly 1), staff +2/3/5/7/9 ATTACK_DAMAGE / greatblade KB +0.1/0.15/0.2/0.25/0.3, consumables live._
- [x] **5.10.x Armor upgrade (2026-09-02 f1da4d0)** — +0..+5 at Workbench ANVIL slot 40 or /armor upgrade, +1 armor/level, shards 10/25/60/150/400 × [1,2,3,4,6] + materials (2R/3R/4R+1A/5R+2A/6R+3A+1C), per-slot identity (helmet speed×2, chestplate HP×2, leggings armor×1.5, boots speed×1.5), old gear +0 compat, RCON verified, config-version 3.
- [ ] **5.10.4 Unstable Rifts + Identifier NPC** — mob loot tables emit rifts; hub NPC stabilizes for a shard fee. _Mob drops wired (2026-08-03): all T2-T4 drop tables emit Unstable Rifts + materials via the `nexo` MythicMobs drop type (migrated from `oraxen` 2026-09-20; Stalker/Phantom/Brute: common/uncommon ~8%, elites: uncommon 30% + rare 15%, bosses: guaranteed high-rarity). Needs live test. Identifier NPC still pending._
...
- [ ] **5.10.7 World population** — spawners, chests, regen emitting rifts from 5.10.4. _DONE in repo (2026-08-10): glitch_red SpawnAreas seeded (T1 fodder everywhere, T2 mid cross-ring, T3 elite guards at Core 0,0 + extraction sites; bootstrap.sh now seeds Spawners/SpawnAreas) + GlitchItems container system (Debris/Cache/Vault/Rift Vault marked per-block, key consumption, regen cooldown, loot-luck consumer). Scatter retuned 2026-09-02 rift_vault 10→6 (141 total, docs/ITEM_BALANCE.md), Vault +5% legendary rift. Remaining: mark containers + extraction arenas in-world (operator) + live test._
...
- [x] **5.12.1 Sellable roster + prices** — _The 20 item configs carry sell-price lore and the shop reads them from config (roll-based gear sell 2026-09-02: base 3/17/75/350/1750 + stars×bonus 2/8/25/90/350, docs/ITEM_BALANCE.md). Deployed 2026-09-02._
