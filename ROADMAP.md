...
- [ ] **5.11.8 Mob health bars (GlitchHealthBar)** — floating color-coded HP bar (██████░░░░ 32/80) above every mob's head. _V1 BUILT (plugins/GlitchHealthBar): TextDisplay above each hostile mob (default mode MONSTERS — covers MythicMobs + vanilla, no plugin internals needed), updates on hit + 1s tick, scales with mob height (boss-safe), config for worlds/bar-length/colors/mode. Needs server build (`sudo ./plugins/GlitchHealthBar/build.sh`) + in-game test._
