...
- [ ] **5.10.6 Residual Glitch** — greed stacks (max 8, +1 every **5 min** — big map, searching is the game, not camping), small loot luck +5%/stack, aggro/risk scaling, extraction payout ×(1+0.10×stacks), elite hunts at 5+ stacks. _V1 built in GlitchItems (timer, boss bar HUD, damage multiplier, payout API) + payout hooked into extraction (GlitchStash pays sell-value × (multiplier−1) on win, stacks clear). Consumers DONE in repo (2026-08-06): loot luck applies at identify (star-luck per roll + rarity-upgrade chance per stack), elite hunt spawns a MythicMobs elite (console mm spawn, configurable mob/interval/radius) while at 5+ stacks. Remaining: container loot-luck consumer with world population (5.10.7)._
