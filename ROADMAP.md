...
**Status as of 2026-08-22:** Phases 0-2 are implemented. Phase 3.1 is configured, but the live Bedrock join test is pending. Phase 4 mechanics are scripted; the default setup creates generated worlds, while imported map saves are external/live-only. **Phase 5.9 is complete in source**: all seven extraction plugins exist (GlitchStash, GlitchClasses, GlitchHideout, GlitchRaid, GlitchInsurance, GlitchEvents, GlitchLoot). All 11 deployable custom plugins plus the shared GlitchCommon library build via the root Maven reactor, and all of them are deployed on the live server and load clean (2026-08-21/22); in-game functional testing remains per [docs/TESTING.md](docs/TESTING.md). GlitchItems/GlitchShops/GlitchHealthBar have full live functional tests; the rest need playtests. GlitchDungeons remains deferred. The repository Standard timer is 30 seconds; generated live arena values still require verification. Oraxen has 18 item definitions and assets. See [docs/STATUS.md](docs/STATUS.md) for the authoritative status. EssentialsX is incompatible with the current Minecraft target. The immediate priorities are in-game verification of the newer plugins, the Identifier NPC flow, physical world content, and end-to-end testing.
